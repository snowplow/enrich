...
class GoogleAnalyticsAdapterSpec extends Specification with DataTables with ValidatedMatchers with CatsIO {
  ...
  def e4 = {
    ...
    toRawEvents(payload, SpecHelpers.client).map(_ must beValid(NonEmptyList.one(RawEvent(api, expectedParams, None, source, context))))
  }
  ...
  def e14 = {
    ...
    toRawEvents(payload, SpecHelpers.client).map(_ must beValid(NonEmptyList.of(event, event)))
  }

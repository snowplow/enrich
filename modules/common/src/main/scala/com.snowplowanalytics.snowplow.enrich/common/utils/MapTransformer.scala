...
object MapTransformer {
  ...
  type TransformFunc = Function2[Key, Value, Either[FailureDetails.SchemaViolation, _]]
  ...
  ): ValidatedNel[FailureDetails.SchemaViolation, T] = {
  ...
  class TransformableClass[T](obj: T)(implicit m: Manifest[T]) {
     ...
     */
    def transform(sourceMap: SourceMap, transformMap: TransformMap): ValidatedNel[FailureDetails.SchemaViolation, Int] =
  ...
  private def _transform[T](
    ...
    transformMap: TransformMap,
    setters: SettersMap
  ): ValidatedNel[FailureDetails.SchemaViolation, Int] = {
    val results: List[Either[FailureDetails.SchemaViolation, Int]] = sourceMap.map {
      case (key, in) =>
    ...
    results.foldLeft(0.validNel[FailureDetails.SchemaViolation]) {
      case (acc, e) =>

...
object MapTransformer {
  ...
  type TransformFunc = Function2[Key, Value, Either[AtomicError, _]]
  ...
  class TransformableClass[T](obj: T)(implicit m: Manifest[T]) {
     ...
     */
    def transform(sourceMap: SourceMap, transformMap: TransformMap): ValidatedNel[AtomicError, Int] =
  ...
  private def _transform[T](
    ...
    transformMap: TransformMap,
    setters: SettersMap
  ): ValidatedNel[AtomicError, Int] = {
    val results: List[Either[AtomicError, Int]] = sourceMap.map {
      case (key, in) =>
    ...
    results.foldLeft(0.validNel[AtomicError]) {
      case (acc, e) =>

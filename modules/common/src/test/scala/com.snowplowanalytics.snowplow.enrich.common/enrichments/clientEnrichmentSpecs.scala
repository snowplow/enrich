...
class ExtractViewDimensionsSpec extends Specification with DataTables {
  ...
  def err: String => AtomicError =
    input => AtomicError(FieldName, Option(input), """Does not conform to regex (\d+)x(\d+)""")

  def err2: String => AtomicError =
    input => AtomicError(FieldName, Option(input), "Could not be converted to java.lang.Integer s")

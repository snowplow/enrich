...
import com.snowplowanalytics.snowplow.eventgen.collector.{CollectorPayload => GenCollectorPayload}
...
//    useragent: Option[String],
  ...
  private def runGen[F[_]: Sync]: F[(GenCollectorPayload, List[Event])] = {
    def go(attempt: Int): F[(GenCollectorPayload, List[Event])] =
      if (attempt >= 5) Sync[F].raiseError(new RuntimeException("Couldn't generate a pair after several attempts"))
      else
        Sync[F].delay(GenSdkEvent.genPair.apply(Gen.Parameters.default, Seed(rng.nextLong()))).flatMap {
  ...
  val client = Client.parseDefault[IO](
    json"""
      {
  ...
  val GeneratorTest = Stream
    .repeatEval(runGen[IO])
    .take(50)
    .flatMap {
      case (payload, events) =>
        Stream
          .emit(ThriftLoader.toCollectorPayload(payload.toRaw, ThriftLoaderSpec.Process))
          .covary[IO]
          .evalTap(_ => IO.delay(println(payload.toString)))
          .through(rethrowBadRows)
          .unNone
          .evalMap(processEvents)
          //          .evalTap(x => IO.delay(println(x.toString)))

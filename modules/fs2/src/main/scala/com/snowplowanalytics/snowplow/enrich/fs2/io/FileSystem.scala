...
object FileSystem {
  ...
  def readJson[F[_]: Sync](path: Path): EitherT[F, String, Json] =
    Sync[F]
      ...
      .attemptT
      .leftMap(e => show"Error reading ${path.toAbsolutePath.toString} JSON from filesystem: ${e.getMessage}")
      .subflatMap(str => parse(str).leftMap(e => show"Cannot parse JSON: ${e.getMessage()}"))
  ...
  def readJsonDir[F[_]: Sync](dir: Path): EitherT[F, String, List[Json]] =
    list(dir).compile.toList.attemptT
      ...
      .flatMap(paths => paths.traverse(x => readJson(x)))
}

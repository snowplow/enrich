...
import com.snowplowanalytics.iglu.core.{SchemaKey, SchemaCriterion, SchemaVer, SelfDescribingData}
...
import com.snowplowanalytics.snowplow.enrich.common.enrichments.registry.apirequest.{Authentication, HttpApi, Cache, Input, Output, JsonOutput}
...
class ApiRequestEnrichmentSpec extends Specification with CatsIO {
  ...
  "ApiRequestEnrichment" should {
    "add a derived context" in {
      ...
      val expected = Contexts(List(SelfDescribingData(
      ...
      testWithHttp.use { test =>
        test.run().map { events => events must beLike {

 ...
 */
package com.snowplowanalytics.snowplow.enrich.pubsub.io
...
import com.snowplowanalytics.snowplow.enrich.pubsub.{AttributedByteSink, AttributedData, ByteSink}
import com.snowplowanalytics.snowplow.enrich.pubsub.config.io.{Authentication, Output}
...
object Sinks {
  ...
  def attributedSink[F[_]: Concurrent: ContextShift: Timer](
    ...
    (auth, output) match {
      ...
      case (_, o: Output.FileSystem) =>
        fileSink(o.file, blocker).map(sink => row => sink(row.data))
    }

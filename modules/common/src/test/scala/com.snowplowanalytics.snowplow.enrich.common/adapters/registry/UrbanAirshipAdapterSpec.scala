...
class UrbanAirshipAdapterSpec extends Specification with ValidatedMatchers {
  ...
  "toRawEvents" should {
    ...
    val actual = UrbanAirshipAdapter.toRawEvents(payload, SpecHelpers.client, SpecHelpers.blocker)
    ...
    "fail on unknown event types" in {
      ...
      UrbanAirshipAdapter.toRawEvents(payload, SpecHelpers.client, SpecHelpers.blocker) must beInvalid
    }
    ...
    "reject content types" in {
      ...
      val res = UrbanAirshipAdapter.toRawEvents(payload, SpecHelpers.client, SpecHelpers.blocker)

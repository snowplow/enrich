...
import cats.effect.{ContextShift, Blocker, Resource, Timer, Concurrent, IO}
...
import com.snowplowanalytics.snowplow.enrich.fs2.{Assets, Environment, EnrichSpec, RawSource, Enrich, Payload}
...
case class TestEnvironment(
   ...
   */
  def run(updateEnv: Environment[IO] => Environment[IO] = identity)
         (implicit C: Concurrent[IO], CS: ContextShift[IO], T: Timer[IO]): IO[List[Either[BadRow, Event]]] = {
    ...
    bad.dequeue.either(good.dequeue)
      .concurrently(stream)
      .haltAfter(5.seconds)
      .compile
      .toList

...
class OlarkAdapterSpec extends Specification with DataTables with ValidatedMatchers with CatsIO {
  ...
  def e1 = {
    ...
    OlarkAdapter.toRawEvents(payload, SpecHelpers.client).map(_ must beValid(expected))
  }
  ...
  def e6 = {
    ...
    OlarkAdapter.toRawEvents(payload, SpecHelpers.client).map(_ must beInvalid(expected))
  }

...
object PayloadGen {
  ...
  val userAgentGen: Gen[String] = for {
    os <- Gen.oneOf("Windows NT 10.0; Win64; x64", "Windows NT 5.1; rv:7.0.1", "Macintosh; Intel Mac OS X 10_14_5", "Macintosh; Intel Mac OS X 10_15_4")
    engine <- Gen.oneOf("AppleWebKit/603.3.8 (KHTML, like Gecko)", "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169", "AppleWebKit/605.1.15 (KHTML, like Gecko)")
  ...
  val byteStream = payloadStream.take(1000).map(p => Chunk.bytes(p.toRaw)).flatMap(Stream.chunk)

  def write(implicit CS: ContextShift[IO]) =
    for {
      counter <- Ref.of[IO, Int](0)
      blocker = Blocker[IO]
      filename = counter.updateAndGet(_ + 1).map(i => Paths.get(s"payload.$i.thrift"))
      _ <- blocker.use { b =>
        val result = payloadStream.flatMap { payload =>
          for {

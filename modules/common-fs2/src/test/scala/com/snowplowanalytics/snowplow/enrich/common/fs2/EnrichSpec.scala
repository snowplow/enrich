...
class EnrichSpec extends Specification with CatsIO with ScalaCheck {
  ...
  "enrichWith" should {
    "enrich a minimal page_view CollectorPayload event without any enrichments enabled" in {
      ...
      Enrich
        .enrichWith(TestEnvironment.enrichmentReg.pure[IO], TestEnvironment.igluClient, None, EnrichSpec.processor, true)(
          EnrichSpec.payload
        )
        .map(normalizeResult)
        .map {
    ...
    "enrich a randomly generated page view event" in {
      ...
      prop { (collectorPayload: CollectorPayload) =>
        ...
        Enrich
          .enrichWith(TestEnvironment.enrichmentReg.pure[IO], TestEnvironment.igluClient, None, EnrichSpec.processor, true)(payload)
          .map(normalizeResult)
          .map {

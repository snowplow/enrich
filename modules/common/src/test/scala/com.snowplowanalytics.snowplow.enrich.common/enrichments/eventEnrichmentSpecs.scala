...
import com.snowplowanalytics.snowplow.enrich.common.enrichments.EventEnrichments.FormatError
  ...
  def err: String => FailureDetails.SchemaViolation =
    input =>
      FailureDetails.SchemaViolation
        .NotJson(FieldName, Option(input), "not recognized as an event type")
...
// format: off
    "SPEC NAME"          || "INPUT VAL"     | "EXPECTED OUTPUT" |
    "None"               !! None            ! FailureDetails.SchemaViolation.NotJson("collector_tstamp", None, "not set").asLeft |
    "Negative timestamp" !! BCTstamp        ! FailureDetails.SchemaViolation.NotJson("collector_tstamp", Some("-0030-01-01T00:00:00.000Z"),"formatted as -0030-01-01 00:00:00.000 is not Redshift-compatible").asLeft |
    ">10k timestamp"     !! FarAwayTstamp   ! FailureDetails.SchemaViolation.NotJson("collector_tstamp", Some("11970-01-01T00:00:00.000Z"),"formatted as 11970-01-01 00:00:00.000 is not Redshift-compatible").asLeft |
...
// format: on
  ...
  def e4 =
    "SPEC NAME" || "INPUT VAL" | "EXPECTED OUTPUT" |
      "Not long" !! (("f", "v")) ! FormatError("f", Some("v"), "not in the expected format: ms since epoch").asLeft |
      "Too long" !! (("f", "1111111111111111")) ! FormatError("f",

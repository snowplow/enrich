...
class IgluAdapterSpec extends Specification with DataTables with ValidatedMatchers with CatsEffect {
  ...
  object Shared {
    val api = CollectorPayload.Api("com.snowplowanalytics.iglu", "v1")
    val cfSource = CollectorPayload.Source("cloudfront", "UTF-8", None)
  ...
  def e8 = {
    ...
    IgluAdapter
      .toRawEvents(payload, SpecHelpers.client, SpecHelpers.registryLookup, SpecHelpers.DefaultMaxJsonDepth)
      .map(_ must beValid(NonEmptyList.one(expected)))
  }

...
case class TestEnvironment[A](
  ...
  def run(
    ...
    val stream = Enrich.run[IO, A](updatedEnv)
      .merge(Assets.run[IO, A](updatedEnv.blocker, updatedEnv.semaphore, updatedEnv.assetsUpdatePeriod, updatedEnv.assetsState, updatedEnv.enrichments))
    for {

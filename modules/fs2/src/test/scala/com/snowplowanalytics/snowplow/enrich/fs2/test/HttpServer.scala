...
object HttpServer extends CatsIO {
  ...
  private def readMaxMindDb(req: Int) = {
    val path =
      if (req < 4) s"/com.snowplowanalytics.snowplow.enrich.fs2/assets-refresh/geoip2-city-$req.mmdb"
      else s"/com.snowplowanalytics.snowplow.enrich.fs2/assets-refresh/geoip2-city-3.mmdb"

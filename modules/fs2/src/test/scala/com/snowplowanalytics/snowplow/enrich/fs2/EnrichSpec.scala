...
import com.snowplowanalytics.snowplow.enrich.fs2.EnrichSpec.{ normalizeResult}
...
//        Enrich
//          .enrichWith(TestEnvironment.enrichmentReg.pure[IO], blocker, TestEnvironment.igluClient, None, _ => IO.unit, None)(
...
//    import concurrent.duration._
    "enrich a randomly generated page view event" in {
      ...
      prop { (collectorPayload: CollectorPayload) =>
        ...
        TestEnvironment.ioBlocker.use { blocker =>
          io.Tracing.build[IO](config.Tracing("http://localhost:9411/api/v2/spans", None)).use { _ =>
            Enrich
              .enrichWith(TestEnvironment.enrichmentReg.pure[IO], blocker, TestEnvironment.igluClient, None, _ => IO.unit, None)(payload)
              .map(normalizeResult)
              .map {
...
//          .map { events =>
//            events must containTheSameElementsAs(List(Right(one), Right(two)))
//          }
//      }
//    }
//  }

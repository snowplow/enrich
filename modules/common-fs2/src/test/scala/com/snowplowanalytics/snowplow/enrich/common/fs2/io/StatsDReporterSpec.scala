...
class StatsDReporterSpec extends Specification {
  ...
  "StatsDeporter" should {
    "serialize metrics" in {
      val snapshot = Metrics.MetricSnapshot(Some(10000L), 10, 20, 30, 40, Some(25), 0, Some(40), Some(0), Some(0))
      ...
      result must contain(
        exactly(
          ...
          "snowplow.test.dropped:40|c|#tag1:abc",
    ...
    "serialize metrics when latency is empty" in {
      val snapshot = Metrics.MetricSnapshot(None, 10, 20, 30, 40, None, 40, Some(40), Some(0), Some(0))
    ...
    "serialize metrics when remote adapter metrics are empty" in {
      val snapshot = Metrics.MetricSnapshot(Some(10000L), 10, 20, 30, 40, Some(25), 40, None, None, None)

...
class PiiPseudonymizerEnrichmentSpec extends Specification with ValidatedMatchers with CatsIO {
  ...
  def commonSetup(enrichmentReg: EnrichmentRegistry[IO]): IO[List[Validated[BadRow, EnrichedEvent]]] = {
    ...
    val client = IgluCirceClient.fromResolver[IO](Resolver(List(reg), None), cacheSize = 0).unsafeRunSync()
    EtlPipeline
      .processEvents[IO](
        new AdapterRegistry(),
        enrichmentReg,
  ...
  private val ipEnrichment = {
    ...
    IpLookupsEnrichment.parse(js, schemaKey, true).toOption.get.enrichment[IO](BlockerF.noop).unsafeRunSync()
  }
  ...
  def e1 = {
    ...
    for {
      size <- output.map(_.size must_== 1)
      validOut <- output.map(_.head must beValid.like {
                    case enrichedEvent =>
  ...
  def e7 = {
    ...
    for {
      size <- output.map(_.size must_== 1)
      validOut <-
        output.map(_.head must beValid.like {
          case enrichedEvent =>
            val contextJ = parse(enrichedEvent.contexts).toOption.get.hcursor.downField("data")
            val firstElem = contextJ.downArray.downField("data")
            val secondElem = contextJ.downArray.right.downField("data")

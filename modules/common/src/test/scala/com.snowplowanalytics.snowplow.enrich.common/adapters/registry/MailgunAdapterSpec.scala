...
class MailgunAdapterSpec extends Specification with DataTables with ValidatedMatchers with CatsIO {
  def is = s2"""
  ...
  def e1 = {
    ...
    MailgunAdapter.toRawEvents(payload, SpecHelpers.client).map(_ must beValid(expected))
  }
  ...
  def e9 = {
    ...
    MailgunAdapter.toRawEvents(payload, SpecHelpers.client).map(_ must beInvalid(expected))
  }

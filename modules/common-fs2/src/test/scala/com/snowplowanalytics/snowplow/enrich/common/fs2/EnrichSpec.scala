      ...
      createIgluClient(List(TestEnvironment.embeddedRegistry)).flatMap { igluClient =>
        Enrich
          .enrichWith(
            ...
            None,
            EnrichSpec.processor,
            EnrichSpec.featureFlags.copy(tryBase64Decoding = false),
            IO.unit
          )(
            Base64.getEncoder.encode(EnrichSpec.payload)
          )
...
object EnrichSpec {
  ...
  val featureFlags = FeatureFlags(acceptInvalid = false, legacyEnrichmentOrder = false, tryBase64Decoding = true)
}

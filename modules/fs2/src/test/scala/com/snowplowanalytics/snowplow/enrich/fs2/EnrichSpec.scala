...
import com.snowplowanalytics.snowplow.enrich.fs2.EnrichSpec.{normalizeResult, Expected, minimalEvent}
  ...
  "enrichWith" should {
    "enrich a minimal page_view CollectorPayload event without any enrichments enabled" in {
      ...
      TestEnvironment.ioBlocker.use { blocker =>
        Enrich
          .enrichWith(TestEnvironment.enrichmentReg.pure[IO], blocker, TestEnvironment.igluClient, None, _ => IO.unit)(EnrichSpec.payload[IO])
          .map(normalizeResult)
          .map {
  ...
  "enrich" should {
    ...
    "enrich event using refreshing MaxMind DB" in {
      ...
      (assetsServer *> TestEnvironment.make(input, List(ipLookupsConf))).use { test =>
        test.run(_.copy(assetsUpdatePeriod = Some(1800.millis)))
          .map { events =>

...
object Enrich {
  ...
  def run[F[_]: Concurrent: ContextShift: Clock](env: Environment[F]): Stream[F, Unit] = {
    ...
    val enrich: Enrich[F] = enrichWith[F](registry, env.igluClient, env.sentry, env.metrics.enrichLatency)
    ...
    env.source
      ...
      .parEvalMapUnordered(ConcurrencyLevel)(payload => env.blocker.blockOn(enrich(payload)))

...
object ConfigFile {
  ...
  implicit val configFileDecoder: Decoder[ConfigFile] =
    deriveConfiguredDecoder[ConfigFile].emap {
      ...
      case c @ ConfigFile(_, Outputs(good, Some(Output.Kinesis(s, _, _, _, _, _, _)), bad), _, _, _, _)
        if(s.isEmpty) => c.copy(output = Outputs(good, None, bad)).asRight
      // Remove pii output if topic empty
      case c @ ConfigFile(_, Outputs(good, Some(Output.PubSub(t, _, _, _, _)), bad), _, _, _, _)
        if(t.isEmpty) => c.copy(output = Outputs(good, None, bad)).asRight
